...
def validate_auth_preconditions():
    ...
    if AWS_PROFILE:
        ...
        return
    # boto3 will also pick up credentials from environment/instance role/ecs task role
...
class S3Fs:
    ...
    def find(self, root: str) -> List[str]:
        ...
        try:
            ...
            page_it = paginator.paginate(Bucket=bucket, Prefix=prefix) if prefix else paginator.paginate(Bucket=bucket)
            for page in page_it:
